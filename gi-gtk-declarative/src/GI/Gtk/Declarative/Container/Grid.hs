...
import           GI.Gtk.Declarative.Unsafe          (unsafePtrEq)

-- | Describes a child widget to be added with 'boxAppend' to a 'Grid'.
data GridChild event = GridChild
...
data GridChildProperties = GridChildProperties
  { left :: Int
  , top :: Int
  , width :: Int
  , height :: Int
...
mkGridChildProperties left top =
  GridChildProperties {left = left, top = top, width = 1, height = 1 }

instance Patchable GridChild where
  create = create . child
  patch s (GridChild props1 child1) gc2@(GridChild props2 child2)
    | props1 `unsafePtrEq` props2 || props1 == props2 = patch s child1 child2
...
instance IsContainer Gtk.Grid GridChild where
  appendChild grid GridChild {properties = GridChildProperties {..}} widget' =
    Gtk.gridAttach grid widget' (fromIntegral left) (fromIntegral top)
